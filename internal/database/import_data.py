...
def import_data(csv_file, vocab_group):
    ...
    for vocab_dict in igcse_vocab_list_of_dicts:
        ...
        shown_english_translation_list = re.split(
            r"[,/](?![^(]*\))", shown_english_translation
        )
        ...
        helpers.new_vocab(
            vocab_word.strip(),
            shown_translations=stripped_shown_english_translation_list,
            all_translations=all_english_translation_list,
            vocab_groups=[vocab_group],
        )
    ...
    print(
        f"{imported_vocab} vocab imported from {csv_file} in {elapsed_time:.4f} seconds"
    )
