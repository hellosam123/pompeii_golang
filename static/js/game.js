...
const difficultyMode = difficultyModeContainer.dataset.difficulty_mode;
const vocabGroupContainer = document.getElementById("vocab-group-info");
const vocabGroup = vocabGroupContainer.dataset.vocab_group;
...
form.addEventListener("submit", async function (event) {
	...
	if (userAnswerInput) {
		isProcessing = true;
		try {
			const answerId = currentVocab.VocabID;
			userAnswer.value = "";
			await loadVocab();
			...
			result = await checkAnswer(answerId, userAnswerInput);
			updateFrame(result);
		} finally {
